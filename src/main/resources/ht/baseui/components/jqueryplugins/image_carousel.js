...
// Create a single carousel item.
    var createItem = function ($image, angle, options) {
        var loaded = false, // Flag to indicate image has loaded.
            ...
            // in options.minScale.
            sizeRange = (1 - options.minScale) * 0.5, // An object to store the public update function.
            that;
...
// Create a carousel.
    var createCarousel = function ($wrap, options) {
        var items = [], rot = 0, pause = false, unpauseTimeout = 0, // Now calculate the amount to rotate per frameRate tick.
            rotAmount = (Math.PI * 2) * (options.frameRate / options.rotRate), $images = $('img', $wrap), // Calculate the angular spacing between items.
            spacing = (Math.PI / $images.length) * 2, // This is the angle of the 1st item at
...
/**
 * Initialization part for Tapestry-jquery.  Note that the syntax is a little different than a prototype mechanism.
 */
(function ($) {
    ...
    T5.extendInitializers(function () {
        ...
        function init(specs) {
            ...
            var pars = specs.config;
            $(specs.targetId).Carousel(pars);
